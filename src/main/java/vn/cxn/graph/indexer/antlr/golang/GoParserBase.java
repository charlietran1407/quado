package vn.cxn.graph.indexer.antlr.golang;
...
public abstract class GoParserBase extends Parser {
    ...
    public void addImportSpec() {
        if (!(this._ctx instanceof GoParser.ImportSpecContext importSpec)) {
            return;
        }
    ...
    // Check if we're in a call to a built-in function that takes a type as first
    // argument.
